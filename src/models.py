...
class ExercisePattern(Enum):
    ...
    PYTHON = "python"                        # Python-based exercises (DynoLabs v5)
...
class Bug:
    ...
    category: str  # Test category that found this bug (TC-PREREQ, TC-GRADE, etc.)
    exercise_id: str
    description: str
...
class TestResult:
    ...
    category: str  # TC-PREREQ, TC-STUDENTSIM, TC-SOL, TC-GRADE, etc.
    exercise_id: str
    passed: bool
    timestamp: str
...
class ExerciseContext:
    ...
    depends_on: List[str] = field(default_factory=list)  # Exercise dependencies
    content_pattern: ExercisePattern = ExercisePattern.UNKNOWN
    course_profile: Optional[Any] = field(default=None, repr=False)  # CourseProfile
...
@dataclass
class CourseResults:
    """Aggregated results for an entire course run (legacy)."""
    course_code: str
    test_date: str
    total_exercises: int
    exercises_tested: int
