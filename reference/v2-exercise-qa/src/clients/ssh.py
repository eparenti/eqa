...
class SSHConnection:
    ...
    def check_lab_package_installed(self, lesson_code: str) -> Tuple[bool, List[str]]:
        """
        Check if a lab package is installed on the workstation.

        Args:
            lesson_code: Lesson code (e.g., 'AU0018L')

        Returns:
            Tuple of (is_installed, list_of_installed_packages)
        """
        # Get list of installed lab packages
        result = self.run("lab list 2>/dev/null || pip list 2>/dev/null | grep -i rht-labs", timeout=30)
        installed_packages = []
        ...
        # Normalize lesson code for comparison (e.g., AU0018L -> au0018l)
        lesson_lower = lesson_code.lower()

        # Check if the package is installed
        is_installed = any(lesson_lower in pkg for pkg in installed_packages)

        return is_installed, installed_packages

    def install_lab_package(self, lesson_code: str, timeout: int = 120) -> CommandResult:
        """
        Install a lab package on the workstation using 'lab force'.
        ...
        Returns:
            CommandResult
        """
        # Use 'lab force' to install the package
        # This syncs the package from PyPI and installs it
        cmd = f"lab force {lesson_code.lower()}"
        print(f"   Installing lab package: {cmd}")
        return self.run(cmd, timeout=timeout)
