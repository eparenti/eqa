...
class EPUBParser:
    ...
    def _find_solution_files(self, exercise_id: str) -> List[Path]:
        ...
        # Collect all files from a solutions directory
        def collect_solutions(solutions_dir: Path):
            if not solutions_dir.exists():
                return
            # Collect ALL files - solutions can be any type:
            # .sol (traditional), .yml/.yaml (playbooks), .j2 (templates),
            # .conf (configs), inventory files (no extension), .sh, .py, etc.
