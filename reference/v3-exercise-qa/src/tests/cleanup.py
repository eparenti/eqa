...
class TC_CLEAN:
    ...
    def test(self, exercise: ExerciseContext, ssh: SSHConnection) -> TestResult:
        ...
        # Step 2: Run lab start (yes | handles conflict resolution)
        print("   → Starting lab...")
        ...
        if not start_result.success:
            bugs_found.append(Bug(
                ...
                category="TC-CLEAN",
                exercise_id=exercise.id,
                description=f"Lab start failed: {(start_result.stdout + start_result.stderr)[:200]}",
        ...
        finish_result = ssh.run_lab_command('finish', exercise.id, timeout=120)
        ...
        restart_result = ssh.run_lab_command('start', exercise.id, timeout=300)
