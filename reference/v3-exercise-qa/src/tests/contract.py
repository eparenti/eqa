...
class TC_CONTRACT:
    ...
    EXPECTED_PATTERNS = {
        'start': {
            'success': [
                r'(SUCCESS|success|started|complete|ready|done)',
        ...
        'finish': {
            'success': [
                r'(SUCCESS|success|finished|complete|cleaned|done)',
        ...
        'grade': {
            'success': [
                ...
                r'(PASS|pass|success|SUCCESS)',
            ],
            'failure': [
                r'(error|FAIL|fail|unable|cannot)',
            ]
        }
    }
    ...
    def _test_start_contract(self, exercise: ExerciseContext, ssh: SSHConnection) -> List[Bug]:
        ...
        # Run lab start via framework-aware method
        result = ssh.run_lab_command('start', exercise.id, timeout=180)

        # Check exit code matches output
        if result.success:
            # Exit code 0 is itself a success indicator.
            # Also check for explicit success text in output.
            ...
            if has_success_indicator:
                ...
                # Exit code 0 counts as success - output may be brief
                # (e.g., lab was already started, or non-TTY output)
                print("      ✓ Start succeeded (exit code 0)")

        else:
            # Check for helpful error message
            err_output = result.stdout + result.stderr
            if len(err_output.strip()) < 20:
                bugs.append(Bug(
    ...
    def _test_grade_contract(self, exercise: ExerciseContext, ssh: SSHConnection) -> List[Bug]:
        ...
        # Run lab grade via framework-aware method
        result = ssh.run_lab_command('grade', exercise.id, timeout=600)
        output = result.stdout + result.stderr

        # Check for score format - accept X/100 or DynoLabs v5 PASS/FAIL checks
        score_pattern = r'(\d+)\s*/\s*100'
        score_match = re.search(score_pattern, output)

        # DynoLabs v5: individual PASS/FAIL check lines
        has_pass_fail = bool(re.search(r'(?:PASS|FAIL)\s{2,}\S', output))

        if not score_match and not has_pass_fail:
            bugs.append(Bug(
                id=f"CONTRACT-GRADE-FORMAT-{exercise.id}",
                severity=BugSeverity.P2_HIGH,
                category="TC-CONTRACT",
                exercise_id=exercise.id,
                description="lab grade output doesn't include score or check results",
                fix_recommendation="Output score as 'X/100' or individual PASS/FAIL checks",
                verification_steps=[
                    f"Run: lab grade {exercise.lab_name}",
                    "Verify output includes score or check results"
                ]
            ))
        elif score_match:
            ...
            pass_count = len(re.findall(r'PASS\s{2,}\S', output))
            fail_count = len(re.findall(r'FAIL\s{2,}\S', output))
            total = pass_count + fail_count
            print(f"      ✓ Grade output includes {total} checks ({pass_count} PASS, {fail_count} FAIL)")
    ...
    def _test_finish_contract(self, exercise: ExerciseContext, ssh: SSHConnection) -> List[Bug]:
        ...
        # Run lab finish via framework-aware method
        result = ssh.run_lab_command('finish', exercise.id, timeout=120)
        ...
        if result.success:
            ...
            if has_success_indicator:
                ...
                print("      ✓ Finish succeeded (exit code 0)")
        else:
            err_output = result.stdout + result.stderr
            if len(err_output.strip()) < 20:
                bugs.append(Bug(
