...
class TC_INSTRUCT:
    ...
    def _find_exercise_content(self, content_dir: Path, exercise_id: str) -> Optional[Tuple[BeautifulSoup, str]]:
        ...
        The exercise ID typically maps to a heading element (h1-h3).
        We need to collect all content from that heading until the next
        ...
        for html_file in list(content_dir.glob("*.xhtml")) + list(content_dir.glob("*.html")):
            try:
                ...
                # If the matched element is a heading (h1-h6), collect all
                # sibling content until the next heading of same/higher level
                if section.name in ('h1', 'h2', 'h3', 'h4', 'h5', 'h6'):
                    heading_level = int(section.name[1])
                    parts = [section.get_text()]
                    ...
                    # Section/div element - use its full content
    ...
    def _check_completeness(self, content: Tuple, exercise: ExerciseContext) -> List[Bug]:
        ...
        for element in required:
            ...
            if element == 'objective':
                # Red Hat Training uses "Outcomes" as the standard term.
                # Also accept: objective, goal, purpose, overview,
                # "in this lab", "in this exercise", "performance checklist"
                objective_terms = [
                    'outcome', 'objective', 'goal', 'purpose', 'overview',
                    'in this lab', 'in this exercise', 'in this guided exercise',
                    'performance checklist', 'you will',
                ]
                if not any(term in text_lower for term in objective_terms):
                    bugs.append(Bug(
                        id=f"INSTRUCT-NOOBJECTIVE-{exercise.id}",
                        severity=BugSeverity.P2_HIGH,
                        category="TC-INSTRUCT",
                        exercise_id=exercise.id,
                        description="Exercise missing clear objective/goal/outcomes statement",
                        fix_recommendation="Add Outcomes section at the beginning",
                        verification_steps=[
        ...
        for sol_file in exercise.solution_files:
            if sol_file.exists():
                try:
                    ...
                    # Only analyze playbooks for modules, not variable files or templates
                    is_playbook = sol_content.lstrip().startswith('---') and \
                        any(kw in sol_content for kw in ['hosts:', 'tasks:', 'roles:'])

                    # Find variable names in Ansible
                    var_pattern = r'{{\s*(\w+)\s*}}'
                    solution_vars.update(re.findall(var_pattern, sol_content))

                    # Find module names (only in playbooks)
                    if is_playbook:
                        module_pattern = r'^\s*(\w+\.\w+\.\w+):\s*$'
